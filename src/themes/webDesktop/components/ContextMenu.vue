  ...
  export default {
    ...
    computed: {
      ...mapState('Platform/' + moduleName, {
        ...
        // 任务栏中的应用列表
        iconMap: state => state.components.desktop.iconMap
      }),
    ...
    watch: {
      'contextMenu.isShow': function (val) {
        let _t = this
        // 判断是否显示菜单 && target为taskBarIcon-app
        if (val && _t.contextMenu.target === 'taskBarIcon-app' && _t.contextMenu.data && _t.contextMenu.data.app.name) {
          // 获取当前app是否已锁定到任务栏
