...
<template>
  <div class="main-page">
    <Tabs v-model="currentTab" class="main-tabs" @on-click="handleCurrentTab">
      ...
      <Tab-pane label="新增" name="add" class="main-tab-pane" v-if="userInfo.type === 0">
        <!-- 账号新增表单 -->
        <EditApplication action="add" v-if="currentTab === 'add'"></EditApplication>
      </Tab-pane>
      <Tab-pane label="编辑" name="edit" class="main-tab-pane" v-if="userInfo.type === 0">
        <!-- 账号编辑表单 -->
  ...
  export default {
    ...
    methods: {
      handleCurrentTab: function (currentTab) {
        let _t = this
        // 切换tab页
        _t.$store.commit('Apps/ApplicationMarket/currentTab/update', currentTab)
      }
    },
    created: function () {
      let _t = this
