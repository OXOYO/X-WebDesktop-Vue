      ...
      <Col span="24" v-if="!isCurrentEditAppInfoEmpty">
        ...
        <Form ref="appForm" :model="formData" :rules="formRules" :label-width="100">
          ...
          <Form-item label="应用名称">
            <Input :disabled="!formData.private" v-model="formData.app_name" placeholder="请输入应用名称"></Input>
          </Form-item>
          <Form-item label="应用标题">
            <Input :disabled="!formData.private" v-model="formData.app_title" placeholder="请输入应用标题"></Input>
          </Form-item>
          <Form-item label="应用描述">
            <Input :disabled="!formData.private" v-model="formData.app_description" type="textarea" placeholder="请输入应用描述"></Input>
          </Form-item>
          <Form-item label="应用类别">
            <RadioGroup v-model="formData.app_type">
          ...
          <Form-item label="私有应用">
            <RadioGroup v-model="formData.private">
              <Radio :label="0" disabled>
                <span>否</span>
              </Radio>
              <Radio :label="1" disabled>
          ...
          <Form-item label="发布应用">
            <RadioGroup v-model="formData.app_publish">
              <Radio :label="0" :disabled="!formData.private">
                <span>否</span>
              </Radio>
              <Radio :label="1" :disabled="!formData.private">
                <span>是</span>
              </Radio>
            </RadioGroup>
          </Form-item>
          ...
          <Form-item label="展现方式">
            <RadioGroup v-model="formData.config.modal.type">
              <Radio label="modal" disabled>
                <span>modal</span>
              </Radio>
              <Radio label="iframe" :disabled="!formData.private">
                <span>iframe</span>
              </Radio>
            </RadioGroup>
          </Form-item>
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        uploadConfig: {
          action: () => {
            let url = _t.$utils.Serv.handleUrl('/MyApplication/icon/upload')
            let query = ''
    ...
    methods: {
      ...
      doSave: async function () {
        ...
        // 2.分发action，执行保存
        let res = await _t.$store.dispatch('Apps/MyApplication/' + _t.action, payload)
        ...
        if (res) {
          ...
          // 切换tab页
          _t.$store.commit('Apps/MyApplication/currentTab/update', 'search')
    ...
    destroyed: function () {
      let _t = this
      // 清空当前编辑的账号信息
      _t.$store.commit('Apps/MyApplication/currentEditAppInfo/update', {})
    }
  }
</script>
